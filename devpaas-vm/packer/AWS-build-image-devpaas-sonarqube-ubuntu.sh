...
########################################################################
#
# title:          Build Ubuntu Image for Sonarqube (Single Instance)
# author:         Marco Maccio (http://marmac.name)
# url:            https://github.com/marcomaccio/devpaas
# description:    Create image for Sonarqube Image server based on image
#
# to run:         sh AWS-build-image-devpaas-sonarqube-ubuntu.sh amazon-ebs \
#
########################################################################
...
if [ -e ./packer_logs/AWS-packer-sonarqube-ubuntu.log ]
then
    echo "Delete the log file of a previous run ..."
    rm -f ./packer_logs/AWS-packer-sonarqube-ubuntu.log
fi

export PACKER_LOG=1
export PACKER_LOG_PATH="./packer_logs/AWS-packer-sonarqube-ubuntu.log"
