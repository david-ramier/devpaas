...
########################################################################
#
# title:          Build Centos7 Image for DevPaas (Single Instance)
...
#
# to run:         sh VBOX-build-image-devpaas-nginx-centos.sh virtualbox-iso
#
########################################################################
...
export PACKER_LOG=1
export PACKER_LOG_PATH="./packer_logs/VBOX-packer-devpass-nginx-centos7.log"
...
echo '****** Build marmac devpaas nginx x centos-7 image ******'
packer build -force -only=$PACKER_PROVIDERS_LIST  $DEBUG            \
        ...
        images/nginx/packer-nginx-centos.json
