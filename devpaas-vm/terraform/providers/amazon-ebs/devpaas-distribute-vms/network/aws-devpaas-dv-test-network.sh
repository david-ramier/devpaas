...
echo "AWS Key path: $HOME/.aws/$AWS_SSH_KEY.pem"
export AWS_KEY_PATH="$HOME/.aws/$AWS_SSH_KEY.pem"
echo "AWS Key path 2: $AWS_KEY_PATH"

echo "Creating a hidden dir to store private key"
ssh -i "${AWS_KEY_PATH}" ubuntu@$JB_PUB_DNS 'mkdir -p .aws'

echo "Copy the private key in the JumpBox to be used to connect to the other VM internally"
scp -i "${AWS_KEY_PATH}" ~/.aws/$AWS_SSH_KEY.pem ubuntu@$JB_PUB_DNS:/.aws/

echo "Testing ICMP Internal traffic from Jump Box to other VMs ..."

echo "Ping the Reverse Proxy VM at the Private IP: $RP_PRIV_IP"
ssh -i "${AWS_KEY_PATH}" ubuntu@$JB_PUB_DNS bash -c "'ping $RP_PRIV_IP -c 2'"

echo "Ping the Front End VM at the Private IP: $FE_PRIV_IP"
ssh -i "${AWS_KEY_PATH}" ubuntu@$JB_PUB_DNS bash -c "'ping $FE_PRIV_IP -c 2'"

echo "Ping the Head End VM at the Private IP: $HE_PRIV_IP"
ssh -i "${AWS_KEY_PATH}" ubuntu@$JB_PUB_DNS bash -c "'ping $HE_PRIV_IP -c 2'"

echo "Ping the DB End VM at the Private IP: $DB_PRIV_IP"
ssh -i "${AWS_KEY_PATH}" ubuntu@$JB_PUB_DNS bash -c "'ping $DB_PRIV_IP -c 2'"

echo "Testing the HTTP Endpoints ... "

echo "Testing the end point on the Jump Box"
curl http://$JB_PUB_DNS
